...
namespace lo
{
    ...
    public:
        ...
        bool voxel_downsample(const typename pcl::PointCloud<PointT>::Ptr &cloud_in, typename pcl::PointCloud<PointT>::Ptr &cloud_out, float voxel_size)
        {
            ...
            // 索引index = x * mul_vx + y * mul_vy + z * mul_vz
...
#pragma omp parallel for //Multi-thread
                                           ...
                                           int keep_number_per_sector, int sector_num)
        {
            ...
            // 遍历所有点 将他们分配在各个sector里面 类似于grid
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                            ...
                            float self_radius, float ghost_radius, float z_min_thre_ghost, float z_min_thre_global)
        {
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                ...
                {
                    // 在剔除落在车身上的点之后，留下较远距离的点云|或者|高于一定高度阈值的点云
        ...
        // -------------------------------------------------------------------------------------------------------------------//
        /**
         * @brief 提取地面及地面法向量和非地面点
         * 
         * @param cloud_in 输入点云
         * @param cloud_ground 地面点
         * @param cloud_ground_down 降采样的地面点
         * @param cloud_unground 非地面点
         * @param cloud_curb 弃用
         * @return true 
         * @return false 
         */
        bool fast_ground_filter(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                                ...
                                int ground_random_down_rate, int ground_random_down_down_rate, int nonground_random_down_rate, int reliable_neighbor_grid_num_thre, // 一些降采样参数
                                ...
                                bool apply_grid_wise_outlier_filter = false, float outlier_std_scale = 3.0) //current intensity_thre is for kitti dataset (TODO: disable it)
        {
            ...
            non_ground_height_thre = appro_mean_height + max_ground_height; // 根据参数表 5m
            ...
            for (int j = 0; j < cloud_in->points.size(); j++)
            {
                ...
                if (temp_id >= 0 && temp_id < num_grid)
                {
                    ...
                    {
                        // 距离中心越远的点的权重越低
                        // tip: 除以一个可以=0的浮点数的时候，加0.00000001可以避免特殊处理
                    ...
                    // 这个是为了剔除一些ghost点 上面作者的说明中提到 雷达驱动会删掉这些点 因此下面这个条件不会额外筛掉
                    // 可能含有地面点
            ...
            {
                //Each grid: Check outlier
                //calculate mean and standard deviation of z in one grid, then set mean-2*std as the threshold for outliers
                // 剔除outliers 高度不在miu - 3 * sigma以上的点 在计算neighbor z的时候这些点不参与
                for (int i = 0; i < num_grid; i++)
                {
                    if (grid[i].pts_count >= min_grid_pt_num)
                    {
                        ...
                        grid[i].min_z_outlier_thre = mean_z - outlier_std_scale * std_z; // 3 sigma
                        grid[i].min_z = max_(grid[i].min_z, grid[i].min_z_outlier_thre); // 在计算neighbor z的时候这些点不参与
            ...
            //Each grid
            // 计算每一个格子的周围格子的最小height 和点大于一定数量的格子的个数
            for (int m = 0; m < num_grid; m++)
            {
                int temp_row, temp_col;
                temp_row = m / col;
...
#pragma omp parallel for
            for (int i = 0; i < num_grid; i++)
            {
                ...
                {
                    // 还是计算降采样ratio 离中心越远的格子的ratio越小 留下点的概率越大
                    // 上一次降采样的是non ground点
                    int ground_random_down_rate_temp = ground_random_down_rate;
                    int nonground_random_down_rate_temp = nonground_random_down_rate;
                    ...
                    if (grid[i].min_z - grid[i].neighbor_min_z < neighbor_height_diff)
                    {
                        ...
                        for (int j = 0; j < grid[i].point_id.size(); j++)
                        {
                            ...
                            if (cloud_in->points[grid[i].point_id[j]].z > grid[i].min_z_outlier_thre)
                            {
                                ...
                                // 地面点之外是非地面点 降采样后放到grid non ground中 这个时候 之前提取出来的很高的点还没有放进去
                    ...
                    if (estimate_ground_normal_method == 3 && grid_ground->points.size() >= min_grid_pt_num)
                    {
                        ...
                        // 实际调用的是pcl的平面拟合 返回来的是ransac后的地面点和法向量
                    ...
                    pcl::PointCloud<PointT>().swap(*grid_ground); // 方法3用完 清空这个指针
                }
            }

            //combine the ground and unground points
            // 把从grid中提取的地面点和非地面点和之前的整合起来 作为返回值 引用传递
            for (int i = 0; i < num_grid; i++)
            {
                                              ...
                                              float dist_thre, int max_iter, float &nx, float &ny, float &nz)
        {
            ...
            // 实际上调用的还是pcl的平面拟合 平面系数存在grid_coeff中， 平面点云存在grid_ground_fit中
        ...
        bool classify_nground_pts(typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                                  ...
                                  float edge_thre /*0.65 */, float planar_thre, float edge_thre_down /*0.75 */, float planar_thre_down,
...

        {
            ...
            // 对应论文 III B 2) 计算特征值 计算并存了一小片点云的linearity planarity curvature
            // cloud feature 存储了每个点附近的点云特征
            pca_estimator.get_pc_pca_feature(cloud_in,
                                             cloud_features,
                                             tree,
                                             neighbor_searching_radius, // 0.6m
                                             neighbor_k,                // 25
                                             1,
                                             pca_down_rate,
            ...
            for (int i = 0; i < cloud_in->points.size(); i++)
            {
                ...
                if (cloud_features[i].pt_num > neigh_k_min)
                {
                    ...
                    if (cloud_features[i].linear_2 > edge_thre)
                    {
                        ...
                        // 如果不用nms的话  edge_thre_down 阈值更高 筛出来的点更少
                    ...
                    else if (cloud_features[i].planar_2 > planar_thre)
                    {
                        ...
                        // 如果不用nms的话  plane_thre_down 阈值更高 筛出来的点更少
            ...
            if (extract_vertex_points_method == 2)
            {
                ...
                for (int i = 0; i < cloud_in->points.size(); i++)
                {
                    ...
                    // 对于一个没有feature的点 附近有足够多的点 且曲率条件可以成为潜在的vertex
            ...
            // 为vertex编码特征
            // 留下的点都是周围有比较丰富特征的 描述子以int形式存在curvature 每两位形容一维特征
...
#pragma omp section
        ...
        bool extract_semantic_pts(cloudblock_Ptr in_block,
                                  ...
                                  float pca_neighbor_radius, int pca_neighbor_k, // 0.6m 25
                                  ...
                                  bool semantic_assisted = false, bool apply_roi_filtering = false, float roi_min_y = 0.0, float roi_max_y = 0.0)
        {
            ...
            else if (apply_scanner_filter)
            {
                ...
                float z_min = -approx_scanner_height - 4.0;                  // 默认 -6m
                float z_min_min = -approx_scanner_height + underground_thre; // 默认-9m
            ...
            // 根据索引随机滤波  这里的参数是留下1024个点  prob = 1 / (size / 1024 + 1)  num = size * prob = 1024 * size / (size + 1024)
		...
		if (apply_roi_filtering) 
		{
            ...
            if (use_adpative_parameters) // false default
        ...
        //Only works on Semantic KITTI dataset (Deprecated) 弃用的
        //http://semantic-kitti.org/dataset.html
