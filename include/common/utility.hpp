...
namespace lo
{
    ...
    struct cloudblock_t
    {
        ...
        pcTPtr pc_raw; //Raw point cloud 原始点云

        pcTPtr pc_down;   //Downsampled point cloud  体素滤波后的点云
        pcTPtr pc_sketch; //very sparse point cloud  随机滤波后的点云

        pcTPtr pc_raw_w; //in world coordinate system (for lidar odometry)

        //unground point cloud
        pcTPtr pc_unground; // 非地面点

        // All kinds of geometric feature points (in target scan)
        pcTPtr pc_ground; // 地面点 及法向量 xyzi + normal
        pcTPtr pc_facade; // 各种特征点
        pcTPtr pc_roof;
        pcTPtr pc_pillar;
        pcTPtr pc_beam;
        pcTPtr pc_vertex;
        ...
        pcTPtr pc_ground_down; // 地面点的降采样
        pcTPtr pc_facade_down; // 各种特征点 降采样
        pcTPtr pc_roof_down;
