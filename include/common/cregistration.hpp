...
#define TEASER_ON 1
...

        {
            ...
            // block1 target是旧点云  block 2 source 是新点云
            //! 取旧点云的稠密特征
            ...
            // init guess 假设是向x移动了5m 应该是T_target_source 将source下的feature转换到target下
            // 但是block中的点云没有变化
            ...
            // 对source cloud降采样 使其不超过target的数量      同时降采样了target中ground和facade点云
...
#pragma omp section
            ...
            for (int i = 0; i < max_iter_num; i++)
            {
                ...
                // T_source_target = T_target_source.inverse()
                inv_init_guess_mat = initial_guess.inverse();
                // 用init guess 做运动补偿 这个时候
...
#pragma omp section
                ...
                // pillar beam facade是必要的correspondence
                ...
                // 计算各种残差和雅克比 得到transform_x  和 对应估计结果的不确定度
                ...
                if (i == max_iter_num - 1 || (i > 2 && ts.norm() < converge_translation && std::abs(rs.angle()) < converge_rotation))
                {
                    ...
                    //sigma_post^2 = (vTPv)/(n-t) = VTPV/(n-t) 在上面计算了
            ...
            registration_cons.sigma = std::sqrt(sigma_square_post);    //Final unit weight standard deviation 归一化标准差
    ...
    private:
        ...
        bool determine_corres(typename pcl::PointCloud<PointT>::Ptr &Source_Cloud,
                              typename pcl::PointCloud<PointT>::Ptr &Target_Cloud,
                              const typename pcl::search::KdTree<PointT>::Ptr &target_kdtree,
                              float dis_thre,
                              boost::shared_ptr<pcl::Correspondences> &Corr_f,
                              bool normal_shooting_on,
                              bool normal_check = true,
                              float angle_thre_degree = 40,
                              bool duplicate_check = true,
                              int K_filter_distant_point = 500)
        {
            int K_min = 3;
                ...
                Target_Cloud->points.size() >= K_min)
            {
                if (normal_shooting_on) // Normal Shooting
                {
                    ...
                    corr_est_ns.determineCorrespondences(
                        *Corr, filter_dis_times * dis_thre); //base on KDtreeNSearch
                                               ...
                                               float pt2pt_residual_window = 0.1, float pt2pl_residual_window = 0.1, float pt2li_residual_window = 0.1)
        {
            ...
            // III C (3 Second  对ground 和roof的权重特殊处理 这样可以让各个方向的优化更平衡  原理不清楚
                                 ...
                                 float residual_window_size = 0.1)
        {
            for (int i = 0; i < (*Corr).size(); i++)
            {
                ...
                if (t_index != -1)
                {
                    ...
                    //以下p'即为变量中的p 变换后的source点   残差是向量内积得到的距离 1维
                    // A = p_d / p_T = p_d / p_p' * (p_p' / p_T)
                    //   = nT * [I -p'^]
                    //   = [nT -p' ^ nT]
                    //   = [nT (n ^ p')T]
                    //   = [ntx nty ntz a b c] 论文（8）
                    // 下面是AT * P * A  // 6 * 1 * 1 * 6
                                               ...
                                               float residual_window_size = 0.1)
        {
            for (int i = 0; i < (*Corr).size(); i++)
            {
                ...
                if (t_index != -1)
                {
                    ...
                    /**
                     * e = v x (p' - q) = u 残差是点 线 垂线段 和点面残差不一样
                     * A = (p_e / p_p') * (p_p' / p_T)
                     * 
                     * p_e / p_p' = v^
                     * p_p' / p_T = [I -p'^]
                     * A = [v^ -v^p'^] = [v^ (vTp')I - pvT]与论文一致 拆开乘一下就知道了
                     * 
                     */
                                 ...
                                 float bbx_pad = 1.0)
        {
            ...
            // 上面取了地面 pillar facade的bbox交集
            // 这里取target点云bbx和init bbx的交集
