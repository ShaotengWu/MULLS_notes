...
namespace lo
{
                                                   ...
                                                   double moving_threshold_tran, double moving_threshold_rot, bool update_edge_or_not)
    {
        ...
        // 构造blocks 并且fixed一些节点 all constraints 都是回环约束
        ...
        // assign the optimized value 更新值 直接更新到all blocks中 也就是所有submap中
...
#define CERES_ON 1
                                               ...
                                               double stable_threshold_tran, double stable_threshold_rot)
    {
        ...
        {
            //fix nodes or not
            if (all_blocks[i]->pose_fixed) // 起始第一帧是固定的
                fixed_node_count++;
        }
        ...
        for (int i = 0; i < all_blocks.size(); i++)
        {
            // 如果有回环帧 在最小回环帧之前的帧设置一个fixed threshold 固定住 只优化后面的部分
            ...
            else
            {
                ...
                // 优化结束后会设置pose stable  优化过的就是stable
        ...
        //when using the diagnoal information matrix, the optimization may not be done (stuck) 为什么？
        return true;
