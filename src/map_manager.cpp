...
namespace lo
{
    // 经过这个函数 target帧的点云被加进了local map
    bool MapManager::update_local_map(cloudblock_Ptr local_map, cloudblock_Ptr last_target_cblock,
                                      float local_map_radius, // 50m
                                      int max_num_pts,        // 8000
                                      int kept_vertex_num, float last_frame_reliable_radius,
                                      bool map_based_dynamic_removal_on, std::string used_feature_type, // "111100", "used_feature_type (1: on, 0: off, order: ground, pillar, beam, facade, roof, vetrex)"
                                      float dynamic_removal_center_radius,                              // 30m
                                      float dynamic_dist_thre_min,
                                      float dynamic_dist_thre_max, float near_dist_thre, // 0.03m
                                      bool recalculate_feature_on)
    {
        ...
        // 对feature做变换 从target到local map（注意这里的变换有个inverse） true代表转换降采样过的特征  false不转换没有降采样过的特征
        last_target_cblock->transform_feature(tran_target_map.inverse(), true, false);

        // min thr是0.3m max thr输入时是0.15乘车速 这里这个区间被限制在0.1m内
        ...
        {
            // filter (0, near_dist_thre) U (dynamic_dist_thre_min, dynamic_dist_thre_max) 筛掉最近邻很近的点和
        ...
        // 把新点云的降采样feature加进来 last_target_cblock中的降采样部分在上面已经做过变换了
        ...
        // 将地图中未降采样的点云从 local map 转换到 target cblock下 这里没有inverse
        ...
        //keep only the points in the sphere  50m
        ...
        //calculate bbx (local) 现在在target cblock坐标系下 原地降采样后 把原来undown的feature添加到raw里（注意读这个函数） raw相当于所有历史feature的总集合
        ...
        // 从当前lidar坐标系到世界坐标系 pose_lo 是位姿 Twm
    ...
    // 筛掉距离local map最近邻 (0, near_dist_thre]近点 U [dynamic_dist_thre_min, dynamic_dist_thre_max]动态点 区间中的点
    ...
    //feature_pts is used to do the next registration (act as target point cloud) [dynamic_dist_thre_down used]
    {
        ...
        // "111100", "used_feature_type (1: on, 0: off, order: ground, pillar, beam, facade, roof, vetrex)"
...
#pragma omp section
    ...
    // keep (near_dist_thre, dynamic_dist_thre_min) U (dynamic_dist_thre_max, +inf)
    bool MapManager::map_scan_feature_pts_distance_removal(pcTPtr feature_pts, const pcTreePtr map_kdtree, float center_radius, // 30m
